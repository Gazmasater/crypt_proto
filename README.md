...
func loadConfig() (Config, error) {
	...
	// DEBUG=true в .env чтобы включить детальные логи
...
const (
	EventBookTicker EventType = iota + 1
	EventUser
	EventUserPB
)
...
type Event struct {
	...
	// Для приватного стрима (текстовое сообщение)
	Raw []byte
}
...
func parsePBWrapperMid(raw []byte) (sym string, mid float64, ok bool) {
	...
	// гарантируем возврат объекта в пул
	defer func() {
...
func runPublicBookTicker(ctx context.Context, wg *sync.WaitGroup, symbol, interval string, out chan<- Event) {
	...
	for {
		...
		retry = baseRetry // сброс бэкоффа

		// дедлайн + PONG handler
		...
		for {
			...
			case websocket.TextMessage:
				// ACK и прочий мусор: логируем только в debug
				dlog("[PUB TEXT] %s", string(raw))
			case websocket.BinaryMessage:
				if sym, mid, ok := parsePBWrapperMid(raw); ok {
					// без fmt.Sprintf / JSON — просто событие
					select {
					case out <- Event{Type: EventBookTicker, Symbol: sym, Mid: mid}:
					case <-ctx.Done():
						close(stopPing)
						_ = conn.Close()
						return
...
func runPrivateUserStream(ctx context.Context, wg *sync.WaitGroup, apiKey, secret string, out chan<- Event) {
	defer wg.Done()
	...
	for {
		...
		for {
			...
			switch mt {
			case websocket.TextMessage:
				// user-событие; если нужно сохранить — копируем
				cp := make([]byte, len(raw))
				copy(cp, raw)
				select {
				case out <- Event{Type: EventUser, Raw: cp}:
				case <-ctx.Done():
					close(stopPing)
					_ = conn.Close()
					return
			...
			case websocket.BinaryMessage:
				// у MEXC user stream тоже бывают pb; пока просто факт
				select {
				case out <- Event{Type: EventUserPB}:
				case <-ctx.Done():
					close(stopPing)
					_ = conn.Close()
					return
...
func main() {
	...
	// консумер событий
	go func() {
		// хранить последние mid по символам
		lastMid := make(map[string]float64)

		// для user-событий — просто печатать как есть (можно потом вынести в отдельную обработку)
		printUser := func(raw []byte) {
			// без лишнего JSON-форматирования
			fmt.Printf("[USER] %s\n", string(raw))
		}

		ticker := time.NewTicker(1 * time.Second) // печатаем агрегированно раз в секунду
		defer ticker.Stop()
		...
		for {
			...
			case ev, ok := <-events:
				...
				case EventUser:
					printUser(ev.Raw)
				case EventUserPB:
					// если нужно — добавить расшифровку
					dlog("[USER.PB] получено бинарное сообщение")
				}
			case <-ticker.C:
				// аккуратно печатаем состояние книги (без спама)
