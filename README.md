...
func NewCalculator(mem *queue.MemoryStore, bookSource collector.DepthBookSource, triangles []*Triangle, oppOut chan<- *executor.Opportunity) *Calculator {
	...
	return &Calculator{
		...
		metricsCfg:    cfg,
		oppOut:        oppOut,
		lastEmit:      make(map[string]emitState),
	}
}

func (c *Calculator) shouldEmitOpportunity(triName string, maxLegTS int64, safeUSDT float64) bool {
	const volumeGrowThreshold = 0.03
...
func (c *Calculator) calcTriangle(md *models.MarketData, tri *Triangle) {
	...
	if c.oppOut != nil && !c.shouldEmitOpportunity(triName, maxLegTS, maxStart) {
		return
	}
