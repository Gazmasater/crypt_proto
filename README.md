...
func printTriangle(t Triangle, profit float64, quotes map[string]Quote) {
	...
	fmt.Fprintf(arbFile, "%s\n", ts)

	// вторая строка — заголовок арба
	fmt.Fprintf(arbFile, "[ARB] %+0.3f%%  %s\n", profit*100, t.Name)

	// дальше — все ноги треугольника, как раньше
	for _, leg := range t.Legs {
		q := quotes[leg.Symbol]
		mid := (q.Bid + q.Ask) / 2
	...
	fmt.Fprintln(arbFile)

	// можно убрать для скорости, если файл не критично сбрасывать мгновенно
	_ = arbFile.Sync()
}
...
func main() {
	...
	// файл для арбитражных треугольников
	var err error
	arbFile, err = os.OpenFile("arbitrage.log",
		os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o644)
	if err != nil {
		log.Fatalf("open arbitrage.log: %v", err)
	}
	defer arbFile.Close()
	...
	[ARB] +0.693%  USDT→KAS→EUR→USDT
  KASUSDT (KAS/USDT): bid=0.0493200000 ask=0.0493560000  spread=0.0000360000 (0.07297%)  bidQty=900.0000 askQty=774.8200
  KASEUR (KAS/EUR): bid=0.0425900000 ask=0.0427200000  spread=0.0001300000 (0.30477%)  bidQty=142.7900 askQty=1093.3100
  EURUSDT (EUR/USDT): bid=1.1683000000 ask=1.1686000000  spread=0.0003000000 (0.02568%)  bidQty=992.9000 askQty=46252.0400
